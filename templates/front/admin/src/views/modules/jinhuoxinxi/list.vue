		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 0px 0","margin":"0px","overflow":"hidden","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit","justifyContent":"flex-end"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0px","margin":"0 10px 20px","borderRadius":"0 0 8px 8px","textAlign":"right","background":"none","display":"block","width":"auto","fontSize":"inherit","order":"1"}' >
					...
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">进货日期</label>
						<el-date-picker v-model="searchForm.jinhuoriqistart" type="date" value-format="yyyy-MM-dd" format="yyyy-MM-dd" placeholder="进货日期起始"></el-date-picker>
						至
						<el-date-picker v-model="searchForm.jinhuoriqiend" type="date" value-format="yyyy-MM-dd" format="yyyy-MM-dd" placeholder="进货日期结束"></el-date-picker>
					</div>
				...
				<el-row class="actions" :style='{"margin":"0px 0 20px","color":"#fff","flexWrap":"wrap","textAlign":"right","flexDirection":"row","display":"flex","width":"auto","fontSize":"inherit","justifyContent":"flex-end","order":"2"}'>
					<el-button class="add" v-if="isAuth('jinhuoxinxi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia17" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('jinhuoxinxi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"border":"0px solid #eee","width":"100%","padding":"0","margin":"0px 0 0","borderRadius":"20px","background":"rgba(255,255,255,1)"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="caipintupian" width="200" label="菜品图片">
						<template slot-scope="scope">
							<div v-if="scope.row.caipintupian">
								<img v-if="scope.row.caipintupian.substring(0,4)=='http'" :src="scope.row.caipintupian.split(',')[0]" width="100" height="100" style="object-fit: cover">
								<img v-else :src="$base.url+scope.row.caipintupian.split(',')[0]" width="100" height="100" style="object-fit: cover">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('jinhuoxinxi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('jinhuoxinxi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('jinhuoxinxi','删除') " type="primary" @click="deleteHandler(scope.row.id )">
...
import AddOrUpdate from "./add-or-update";
    ...
    params['sort'] = 'jinhuoriqi';
    params['order'] = 'desc';
           if(this.searchForm.jinhuoriqistart!='' && this.searchForm.jinhuoriqistart!=undefined ){
            params['jinhuoriqistart'] = this.searchForm.jinhuoriqistart
          }
          if(this.searchForm.jinhuoriqiend!='' && this.searchForm.jinhuoriqiend!=undefined){
            params['jinhuoriqiend'] = this.searchForm.jinhuoriqiend
          }
	...
	jinhuoxinxistatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'jinhuoxinxi/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
