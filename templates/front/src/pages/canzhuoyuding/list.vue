...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"0 0 0px","margin":"10px 0 0","borderColor":"#ddd","alignItems":"center","display":"flex","clear":"both","justifyContent":"","flexWrap":"wrap","background":"none","borderWidth":"0 0 0px","width":"100%","borderStyle":"solid","height":"auto","order":"1"}'>
      <el-form-item :style='{"margin":"0 5px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>餐桌类型：</div>
        <el-input v-model="formSearch.canzhuoleixing" placeholder="餐桌类型" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 5px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>就餐时间：</div>
        <el-date-picker 
          v-model="timeRange"
          type="datetimerange"
          range-separator="至"
		  :style='{"padding":"0px 10px","borderColor":"#ddd","borderRadius":"0px","background":"#fff","borderWidth":"1px","width":"auto","lineHeight":"36px","borderStyle":"solid","justifyContent":"center","height":"36px","maxWidth":"390px"}'
          start-placeholder="就餐时间起始"
          end-placeholder="就餐时间结束"
	  ...
	  <el-button v-if="btnAuth('canzhuoyuding','新增')" :style='{"cursor":"pointer","border":"1px solid #ccc","padding":"0px 8px","margin":"0px 10px 0 0","color":"#666","borderRadius":"0px","background":"none","display":"inline-block","width":"auto","fontSize":"14px","lineHeight":"36px","height":"36px"}' type="primary" @click="add('/index/canzhuoyudingAdd')"><i v-if="true" :style='{"color":"#666","margin":"0 2px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"width":"calc(100% - 0px)","padding":"20px 0 0 0","margin":"0","background":"none","flex":"1","order":"6"}'>
		...
		<div class="list1 index-pv1" :style='{"padding":"0 0 0 5px","margin":"10px 0 0","overflow":"hidden","color":"#666","alignItems":"flex-start","flexWrap":"wrap","background":"none","display":"flex","width":"100%","fontSize":"14px","height":"auto"}'>
			<div :style='{"border":"0px solid #ddd","cursor":"pointer","padding":"0","margin":"0 30px 30px 0","overflow":"hidden","flexWrap":"wrap","background":"#fff","display":"flex","width":"calc(25% - 30px)","position":"relative","justifyContent":"center","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
				<img @click.stop="imgPreView(item.canzhuotupian)" :style='{"border":"0px solid #eee","padding":"10px","margin":"0 0 10px","objectFit":"contain","background":"#f4f4f4","display":"block","width":"100%","height":"245px"}' v-if="item.canzhuotupian && item.canzhuotupian.substr(0,4)=='http'" :src="item.canzhuotupian.split(',')[0]" class="image" />
				<img @click.stop="imgPreView(baseUrl + (item.canzhuotupian?item.canzhuotupian.split(',')[0]:''))" :style='{"border":"0px solid #eee","padding":"10px","margin":"0 0 10px","objectFit":"contain","background":"#f4f4f4","display":"block","width":"100%","height":"245px"}' v-else :src="baseUrl + (item.canzhuotupian?item.canzhuotupian.split(',')[0]:'')" class="image" />
				...
				<div :style='{"padding":"0 0px","borderColor":"#ddd","margin":"0px","whiteSpace":"nowrap","color":"#5A5A5A","textAlign":"center","overflow":"hidden","borderWidth":"0 0 0px","width":"100%","lineHeight":"30px","fontSize":"16px","textOverflow":"ellipsis","borderStyle":"solid"}' class="name ">{{item.canzhuobianhao}}</div>
				<div :style='{"padding":"0 0px","borderColor":"#ddd","margin":"0px","whiteSpace":"nowrap","color":"#5A5A5A","textAlign":"center","overflow":"hidden","borderWidth":"0 0 0px","width":"100%","lineHeight":"30px","fontSize":"16px","textOverflow":"ellipsis","borderStyle":"solid"}' class="name ">餐桌类型:{{item.canzhuoleixing}}</div>
				<div :style='{"padding":"0 0px","borderColor":"#ddd","margin":"0px","whiteSpace":"nowrap","color":"#5A5A5A","textAlign":"center","overflow":"hidden","borderWidth":"0 0 0px","width":"100%","lineHeight":"30px","fontSize":"16px","textOverflow":"ellipsis","borderStyle":"solid"}' class="name ">{{item.jiucanshijian}}</div>
				...
				<div :style='{"padding":"0 0px","margin":"0 5px 0 0","display":"inline-block","order":"5"}'>
				  ...
				  <span class="text" :style='{"color":"#999","lineHeight":"1.5","fontSize":"inherit"}'>{{item.yonghuming}}</span>
				</div>
			</div>
		</div>
  ...
  export default {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.canzhuoleixing != '') searchWhere.canzhuoleixing = '%' + this.formSearch.canzhuoleixing + '%';
        if (this.timeRange.length > 0) {
          searchWhere.jiucanshijianstart = this.timeRange[0];
          searchWhere.jiucanshijianend = this.timeRange[1];
        }
		if(!this.centerType){
			params['sfsh'] = '是';
		}
        ...
        this.$http.get(`canzhuoyuding/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/canzhuoyudingDetail', query: params});
      },
